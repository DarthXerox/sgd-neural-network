...
private:
        ...
        forward_prop_backup[0] = input.get_pixels();
            for (size_t i = 1; i <= layer_sizes.size(); ++i) {
                ...
                if(i != layer_sizes.size()){
                    inner_potential[i - 1] = forward_prop_backup[i];
                }
                ...
                for(size_t i = 0; i < forward_prop_backup[l_size - 1].size(); i++){
                    ...
                    for(size_t j = 0; j < forward_prop_backup[l_size].size();j++){
                        out_sum += output[l_size][j] *
                                ...
                                (activation_functions[l_size - 1], inner_potential[l_size - 1][j]) *
                                layers[l_size - 1].get_weight(i,j);
                    }
                    output[l_size - 1][i] = out_sum;
                }
    ...
    void prepare_backup() {
        inner_potential.push_back(std::vector<F>());
        forward_prop_backup.push_back(std::vector<F>());
        output.push_back(std::vector<F>(input_layer_size));
        for (size_t i = 0; i < layer_sizes.size() - 1; ++i) {
            output.push_back(std::vector<F>(layer_sizes[i]));
        }
        for (size_t i = 2; i < layer_sizes.size(); ++i) {
            inner_potential.push_back(std::vector<F>(layer_sizes[i - 1]));
        }
    ...
    std::vector<std::vector<F>> inner_potential;
