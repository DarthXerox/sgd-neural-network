...
template<typename F = float>
struct WeightLayer {
    WeightLayer(int lower_size, int upper_size) {
        weights = std::vector<std::vector<F>>(lower_size);
        delta_weights = std::vector<std::vector<F>>(lower_size, std::vector<F>(upper_size, 0));
        delta_biases = std::vector<F>(upper_size, 0);
...
//        srand(SEED);
    ...
    void correct_weights(const std::vector<std::vector<F>> &gradients, const std::vector<F> &bias_gradients, F lr, F q) {
//#pragma omp parallel num_threads(8)
//        {
//#pragma omp for collapse(2)
            for (size_t i = 0; i < weights.size(); ++i) {
                for (size_t j = 0; j < weights.front().size(); ++j) {
                    delta_weights[i][j] = (1 - q) * gradients[i][j] + q * (delta_weights[i][j]);
                    weights[i][j] -= ((lr * delta_weights[i][j]) + (1.0f - DECAY) * weights[i][j]);
                }
            }

//#pragma omp for
            for (int i = 0; i < biases.size(); ++i) {
                delta_biases[i] = (1 - q) * bias_gradients[i] + q * (delta_biases[i]);
                biases[i] -= ((lr * delta_biases[i]) + (1.0f - DECAY) * biases[i]);
            }
//        }
        get_transposed_weights();
    }
...
private:
   ...
   std::vector<std::vector<F>> delta_weights;
   std::vector<std::vector<F>> weights;
   std::vector<std::vector<F>> transposed_weights;

   std::vector<F> delta_biases;
   std::vector<F> biases; // for the upper layer
